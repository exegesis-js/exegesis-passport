...
import {
  default as exegesisPassport,
  PassportUserToExegesisUserFn,
...
describe("integration", function () {
  for (const withPassport of [true, false]) {
    for (const converter of [converterFn, undefined]) {
      describe(`${withObj(withPassport)} passport, ${withObj(
        converter
      )} conveter`, function () {
        ...
        it("should deny a call to the API with missing api key", async function () {
          const fetch = makeFetch(this.server);
          await fetch(`/greet?name=Jason`).expect(401);
        });
        ...
        it("should convert user from strategy into an exegesis user", async function () {
          ...
          const expected = converter
            ? expectedWithConverter
            : expectedWithoutConverter;
          const fetch = makeFetch(this.server);
          await fetch(`/whoami`, { headers: { apiKey: "secret" } }).expect(
            200,
            expected
          );
        });

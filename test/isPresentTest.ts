...
function makeApiKeyAuthenticator(opts: Options = {}) {
  ...
  const authenticator: CallbackAuthenticator = exegesisPassport(
    passport,
    "apiKey",
    opts
  );
...
describe("isPresent", function () {
  describe("explicit", function () {
    it("should explicitly detect missing credentials", async function () {
      ...
      const missingResult = await pb.call((done: any) =>
        authenticator(pluginContext, {}, done)
      );
      ...
      const invalidResult = await pb.call((done: any) =>
        authenticator(pluginContext, {}, done)
      );
    ...
    it("should work when `isPresent()` is full of lies", async function () {
      ...
      const result = await pb.call((done: any) =>
        authenticator(pluginContext, {}, done)
      );

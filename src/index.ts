...
export type PassportToExegesisResult = Pick<
  AuthenticationSuccess,
  "user" | "roles" | "scopes"
> & { [prop: string]: any };

export interface PassportUserToExegesisUserFn {
  (user: any, pluginContext: ExegesisPluginContext): PassportToExegesisResult;
}

export interface IsPresentFn {
  (pluginContext: ExegesisPluginContext, info: AuthenticatorInfo): boolean;
}
...
function defaultIsPresent(
  ...
  if (
    info.name &&
    info.in === "header" &&
    !(info.name.toLowerCase() in context.req.headers)
  ) {
    answer = false;
  } else if (
    info.name &&
    info.in === "query" &&
    !context.req.url?.includes(info.name)
  ) {
    answer = false;
  } else if (info.scheme) {
    ...
    if (authorization === null || authorization === undefined) {
      ...
      return (
        normalizedHeader === scheme || normalizedHeader.startsWith(`${scheme} `)
      );
    }
  }
  ...
  return function passportAuthenticator(
    ...
    const origDone = clearUser(req, done);
      ...
      if (err) {
        ...
        origDone(
          null,
          generateSuccessResult(pluginContext, converter, req.user)
        );
      } else {
        origDone(err, result);
      }
      ...
      (err, result) => {
        ...
        switch (result.type) {
          case "success": {
              ...
              null,
              generateSuccessResult(pluginContext, converter, result.user)
            );
            break;
          }
          case "pass":
